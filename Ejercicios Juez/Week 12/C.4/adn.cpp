...
int adn(int n, int m, string s1, string s2, Matriz<int> punt) {
  ...
  for(int i = 0; i < n; i++) {
    for(int j = 0; j < m; j++) {
      int indiceI = calcularIndice(s1[i]);
      int indiceJ = calcularIndice(s2[j]);
      if(i + j == 0) sol[i][j] = 0;
      else if (i == 0) sol[i][j] = punt[4][indiceJ] + sol[0][j - 1];
      else if (j == 0) sol[i][j] = punt[indiceI][4] + sol[i - 1][0];
      else {
        sol[i][j] = max(max(punt[indiceI][4] + sol[i - 1][j], punt[4][indiceJ] + sol[i][j - 1]), punt[indiceI][indiceJ] + sol[i - 1][j - 1]);
      }
